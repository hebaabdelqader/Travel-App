const fetchCoordinates = async(location) => {
    const response = await fetch(`http://api.geonames.org/searchJSON?q=${location}&maxRows=1&username=${process.env.fetchCoordinatesKey}`);
    if (!response.ok) {
...
const fetchWeather = async (lat, lng) => {
    const response = await fetch(`https://api.weatherbit.io/v2.0/forecast/daily?lat=${lat}&lon=${lng}&key=${process.env.fetchWeatherKey}`);
    const data = await response.json();
    return data;
}

const fetchImage = async (location) => {
    const response = await fetch(`https://pixabay.com/api/?key=${process.env.fetchImageKey}&q=${encodeURIComponent(location)}&image_type=photo`);
    const data = await response.json();
